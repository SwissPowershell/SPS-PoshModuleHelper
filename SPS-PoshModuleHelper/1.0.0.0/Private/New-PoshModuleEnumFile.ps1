...
Function New-PoshModuleEnumFile {
    [CMDLetBinding()]
    Param(
        [Parameter(
            Position = 0,
...
"@
    ...
    PROCESS {
        ...
        #region Function Processing DO NOT REMOVE
        Write-Verbose "Creating Enum '$($Name)' file "
        Try {
            $Null = Set-Content -Path $Path -Value $EnumContent -Force
        }Catch {
            Throw "Unable to create Enum '$($Name)' file : $($_.Exception.Message)"
        }
    }
    END {
